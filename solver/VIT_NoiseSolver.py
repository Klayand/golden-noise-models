...
class Vit_Solver:
    ...
    def train(
            self, train_loader: DataLoader, val_loader, total_epoch=5000
    ):
        ...
        for epoch in range(1, total_epoch + 1):
            ...
            for step, (original_noise, optimized_noise, prompt) in enumerate(pbar, 1):
                ...
                # loss = self.alpha * mse_loss(golden_noise, optimized_noise) + self.beta * mse_loss(predicted_img.flatten(), optimized_img.flatten())
            ...
            with torch.no_grad():
                ...
                print("Eval Loss:", round(total_eval_loss.item() / count, 3))

        torch.save({"unet": self.unet.state_dict(), "embeeding": self.text_embedding.state_dict()}, "./golden_unet.pth")
        return self.unet
